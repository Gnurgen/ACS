...
\textbf{latency}
 Figure 2 shows the latency at various amounts of concurrent clients when running the test locally and using RPC. Again the numbers for one measurement are much greater than the others, in this case RPC is greather than the local measurements. But here the trend does not look like it is the same. The increase in latency for the local measurements looks to be about linear from the data. At 10 clients the latency is 1 ms per instruction and at 100 clients the latency is roughly 10 ms per instruction. The Latency for RPC on the other hand looks to be slowly growing exponentially. This could be because the overhead added by RPC means that fewer clients are needed before the tendency for exponential growth is visible, but we need to go beyond 100 clients to see the same tendency in the local measurements.
